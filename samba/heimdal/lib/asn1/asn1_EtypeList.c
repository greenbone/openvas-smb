...
ret = 0;
e = encode_krb5int32(p, len, &(data)->val[i], &l);
if (e) return e;
p -= l; len -= l; ret += l;
...
e = decode_krb5int32(p, len, &(data)->val[(data)->len], &l);
if(e) goto fail;
p += l; len -= l; ret += l;
(data)->len++;
...
{
while((data)->len){
free_krb5int32(&(data)->val[(data)->len-1]);
(data)->len--;
}
free((data)->val);
(data)->val = NULL;
...
ret = 0;
ret += length_krb5int32(&(data)->val[i]);
ret += Top_tag_for_oldret;
}
ret += Top_tag_oldret;
}
...
if(copy_krb5int32(&(from)->val[(to)->len], &(to)->val[(to)->len])) goto fail;
}
return 0;
fail:
free_EtypeList(to);
